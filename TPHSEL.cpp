...
#define NITRS 5		//number of iterations
...
unsigned short dset, minimum_generations;
...
char this_class[MAXCLASSES][100];
unsigned short class_is[2][MAXSAMPLES], class_found[MAXSAMPLES], class_size[MAXCLASSES], chrom_length;
unsigned short tr_size, ts_size, trl_size, num_samples, num_classes, elite, level, num_features;
unsigned short trs[MAXSAMPLES], tss[MAXSAMPLES], trsl[MAXSAMPLES];
...
public:
	...
	bool similarity_index_check(unsigned short* sample1, unsigned short* sample2)
	{
		...
		for (spl1 = 1; spl1 < tr_size; spl1++)
		{
			...
			{
				if (class_is[0][trs[spl2]] == class_is[0][trs[spl1]] or class_is[0][trs[spl2]] == class_is[1][trs[spl1]]) continue;
				similarity_index = 0;
	...
	{
		class_is[1][trs[spl1]] = class_is[0][trs[spl2]];
		class_is[1][trs[spl2]] = class_is[0][trs[spl1]];
	}
...
public:
	...
	double max(double* PSLW, unsigned short id, unsigned short& smpl)
	{
		double mxm(-VHV);
		for (spl = 1; spl <= tr_size; spl++)
		{
			if (class_is[0][trs[spl]] != id) continue;
			if (PSLW[trs[spl]] > mxm)mxm = PSLW[trs[spl]];
		}
		return mxm;
	}
	// finding minimum value of the class
	double min(double* PSLW, unsigned short id, unsigned short& smpl)
	{
		double mnm(VHV);
		for (spl = 1; spl <= tr_size; spl++)
		{
			if (class_is[0][trs[spl]] != id) continue;
			if (PSLW[trs[spl]] < mnm)mnm = PSLW[trs[spl]];
		}
		return mnm;
	}
	//finding mean of the sample
	double mean(double* PSLW, unsigned short id, unsigned short& smpl)
	{
		double sum(0.0);
		for (spl = 1; spl <= tr_size; spl++)
		{
	...
	//standard deviation
	double standard_deviation(double* PSLW, double mean, unsigned short id, unsigned short smpl)
	{
		double sum(0.0);
		for (spl = 1; spl <= tr_size; spl++)
		{
			if (class_is[0][trs[spl]] != id) continue;
			sum += pow(PSLW[trs[spl]] - mean, 2);
		}
		if (smpl == 0) return 0;
...
public:
	//classification
	unsigned short find_class(double* prb, unsigned short* numsamples, double set_partition)
	{
	...
	{
		if (two_values_are_equal(dim, max) and two_values_are_equal(dim, min)) return 1; //if all the members have same value
	...
	void test_set()
	{
		...
		unsigned short cls, spl, id, lvl, numsamples[MAXCLASSES];
		for (cls = 1; cls <= num_classes; cls++)
			numsamples[cls] = 1;		//number of samples in each class are greater than 0
		for (spl = 1; spl <= ts_size; spl++)
		{
			for (lvl = 1; lvl <= level; lvl++)
			{
				...
				class_found[tss[spl]] = find_class(prb[lvl], numsamples, model_set_part[lvl]);
			...
			{
				class_found[tss[spl]] = class_is[0][trs[1]];
				vote[class_found[tss[spl]]][tss[spl]] += 1;
	...
	void set_partition()
	{
		...
		stats.statistical_parameters(value);				//finding minimum, maximum, mean and standard deviation of all features of the training set
		for (spl = 1; spl <= tr_size; spl++)
		{
			for (cls = 1; cls <= num_classes; cls++)
				prb[cls] = probability(value[trs[spl]], maximum[cls], minimum[cls], average[cls]);
			guess = find_class(prb, numsamples, set_part[elite]);
...
public:
	...
	void operator_selection()
	{
		...
		do
		{
			chrom++;
			if (chrom == elite)
				oprtr_select[chrom] = 2;	//elite cloning
...
public:
	...
	unsigned short fitness_evaluation()
	{
		...
		{
			prb_cover[phrom] = 1.0;
			for (unsigned short i = 1; i <= num_classes; i++)
				numsamps_fit[i] = 0;
			set_part[phrom] = sample_fitness[phrom] = class_fitness[phrom] = 0;
			for (spl = 1; spl <= tr_size; spl++)
				value[trs[spl]] = eva.evaluate(chromosome[phrom], trs[spl]);
			stats.statistical_parameters(value);				//finding minimum, maximum, mean and standard deviation of all features of the training set
			for (spl = 1; spl <= tr_size; spl++)
			{
				for (cls = 1; cls <= num_classes; cls++)
				...
				guess = clf.find_class(prb, numsamples, set_part[phrom]);
				if (guess == class_is[0][trs[spl]] or guess == class_is[1][trs[spl]])
				{
					numsamps_fit[guess]++;
					for (cls = 1; cls <= num_classes; cls++)
					{
						if (cls != guess)
							prb_cover[phrom] = __min(prb[guess] - prb[cls] - set_part[phrom], prb_cover[phrom]);
					}
				}
				else if (guess <= num_classes)
				{
					set_part[phrom] = prb[guess] - prb[class_is[0][trs[spl]]] + PRECISION;
					spl = 0;
					for (unsigned short i = 1; i <= num_classes; i++)
						numsamps_fit[i] = 0;
				}
...
public:
	...
	//generating bagging set
	void bagging_set()
	{
		tr_size = trl_size;
		for (spl = 1; spl <= tr_size; spl++)
			trs[spl] = trsl[1 + rand() % trl_size];
	}
	...
	void subsets(unsigned short num_folds)
	{
		...
		{
			bool fold_examined[11]{};
			nf = 0;
			do
			{
				do
					fold = 1 + rand() % num_folds;
				while (fold_examined[fold] and nf < num_folds);
				fold_examined[fold] = true;
				nf++;
			} while (nf <= num_folds and (n_spl[fold] >= num_samples / float(num_folds) or num_spl[fold][class_is[0][spl]] >= class_size[class_is[0][spl]] / float(num_folds)));
			if (nf > num_folds)
			{
				do
					fold = 1 + rand() % num_folds;
				while (n_spl[fold] >= num_samples / float(num_folds));   //first priority constraint
			}
			n_spl[fold]++;  //increment in number of samples in this fold
			num_spl[fold][class_is[0][spl]]++;      //increment in number of samples of this class in this fold
...
void parser(string dataset)
{
	string file_name;
	file_name = dataset;
	ifstream fin(file_name);        //object
	bool done(false);
	char instr[200];
	unsigned short spl, ftr, word, cls;
	while (!done)
	{
		fin >> instr;
		word = whatis(instr);
		...
		case 3: fin >> instr;
			...
			for (spl = 1; spl <= num_samples; spl++)
			{
				...
				fin >> instr;
				class_is[0][spl] = instr[1] - '0';
				class_size[class_is[0][spl]]++;
			}
			break;
...
unsigned short ensemble_results(ofstream& fout)
{
        ...
        for (spl = 1; spl <= num_samples; spl++)
        {
                ...
                fout << cls_elected[1] << "\t" << class_is[0][spl] << "\n";
                if (class_is[0][spl] == cls_elected[1]) num_correct++;
        }
        ...
        fout << num_correct / double(num_samples) * 100;
        fout << "\n";
}
...
int main(int argc, char *argv[])
{
	...
	ofstream fout(fname.c_str());	//training file object
	fname = "test_results.xls";
	ofstream file_out(fname.c_str());      //test file object
	fname = "ensemble_results.xls";
	ofstream f_out(fname.c_str());		//ensemble file object
	...
	for (fold = 1; fold <= num_folds; fold++)
	{
		...
		for (itr = 1; itr <= NITRS; itr++)
		{
			...
			clf.test_set();
			file_out << "fold#" << "\t" << "iteration#" << "\t" << "sample#" << "\t" << "predicted_class" << "\t" << "actual_class" << "\t" << "decision_level" << "\n";
			for (unsigned short spl = 1; spl <= ts_size; spl++)
				file_out << fold << "\t" << itr << "\t" << tss[spl] << "\t" << class_found[tss[spl]] << "\t" << class_is[0][tss[spl]] << "\t" << test_level[tss[spl]] << "\n";
		}
	}
	ensemble_results(f_out);
	end_time = clock();
	f_out << "Execution Time = " << (double)(end_time - start_time) / CLOCKS_PER_SEC << "\n";
	return 0;
}
